...
/**
 * Combine an array of image buffers into a single PNG `Buffer`.
 *
 * Images are placed in the order they appear in the `buffers` array.
 ...
 *   Raw binary data for each source image.  Any format that libvips/sharp
 *   understands is accepted (JPEG, PNG, WEBP, AVIF, TIFF, GIF, …).
 ...
 *
 * @example
 * // Combine two JPEG files side by side
 * const fs = require('fs');
 * const { combineImages } = require('./lib/combineImages');
 *
 * const buffers = [
 *   fs.readFileSync('left.jpg'),
 *   fs.readFileSync('right.jpg'),
 * ];
 * const png = await combineImages(buffers, 'horizontal');
 * fs.writeFileSync('combined.png', png);
 */
